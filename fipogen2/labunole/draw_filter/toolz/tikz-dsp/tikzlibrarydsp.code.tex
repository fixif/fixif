...
% Modified by Joachim Kruithof 2015
% Add triangle block
% Change some sizes to get a visually-even result
%
...
\usetikzlibrary{arrows, calc, positioning, decorations.markings, shapes.geometric} % needs shapes.geometric for triangle
...
\newcommand{\dspoperatordiameter}{6mm}       % Diameter for adder, multiplier, mixer
\newcommand{\dspoperatorlabelspacing}{1.4mm}   % Distance from symbol to label for adder, multiplier, mixer
\newcommand{\dspnoderadius}{2mm}             % Filled and empty node
\newcommand{\dspsquareblocksize}{10mm}        % Size for square blocks, e.g. for delay elements, decimator, expander
\newcommand{\dspfilterwidth}{14mm}           % Width of a filter block
\newcommand{\dsptrianglewidth}{10mm}           % Width of a triangle filter block
...
% Generic shape generator for circle operators, i.e. nodes with a circular
% shape with an additional customizable drawing and a text label
\long\def\dspDeclareCircleOperator#1#2{
	\pgfdeclareshape{#1}
	{
	  ...
	  \anchor{text}
	  {
	    ...
	    \def\templabelaboveright{aboveright}
	    \def\templabelbelowright{belowright}
	    ...
	    \fi
	    %
	    \ifx\label\templabelaboveright
				\advance\pgf@y by \radius
				\advance\pgf@x by \pgfutil@tempdima
	    \fi
	    %
	    \ifx\label\templabelbelowright
	    			\advance\pgf@y by -\radius
	    			\advance\pgf@x by \pgfutil@tempdima
				\advance\pgf@y by -0.75\ht\pgfnodeparttextbox
				%\advance\pgf@y by 
	    \fi
	  }
	}
...
% Generic shape operator for a dsp triangle operator with a label
% 
% \long\def\dspDeclareTriangleOperator#1#2{
% 	\pgfdeclareshape{#1}
% 	{
% 		
% 
...
\dspDeclareCircleOperator{dspshapecircle}{\pgfusepathqstroke}

\dspDeclareCircleOperator{dspshapecirclefull}{\pgfusepathqfillstroke}

\dspDeclareCircleOperator{dspshapeadder}{
...
\dspDeclareCircleOperator{dspshapemixer}{
...
% Triangle multiplier (Left, Right, Up, Down)
\tikzset{dsptriangle/.style={shape=isosceles triangle, draw, shape border rotate = #1, align=center, text depth=0.3em,text height=1em,inner sep=0pt,
	line cap=round,line join=round,line width=\dsplinewidth,minimum height=\dspsquareblocksize,minimum width=\dspsquareblocksize}}
\tikzset{dsprtriangle/.style={dsptriangle=0}}
\tikzset{dspltriangle/.style={dsptriangle=180}}
\tikzset{dsputriangle/.style={dsptriangle=+90}}
\tikzset{dspdtriangle/.style={dsptriangle=-90}}
% -------------------------------------------------------------------------
...
\tikzset{dspdconn/.style={dashed,->,>=dsparrow,line width=\dsplinewidth},line cap=round,line join=round}
...
% Add other signal flow lines
% discontinuous line
\tikzset{dspdline/.style={dashed, line width=\dsplinewidth, line cap = round, line join=round}}
