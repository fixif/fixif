...
class test_LTI( unittest.TestCase):
	...
	def test_Gramians(self):
		...
		for i in range(50):
			...
			S._W_method = 'linalg'
			my_assert_relativeclose( test_LTI.msgH,
...
# 				 w = res + absolute(S.D)
		...
		# global recollection of error messages
		if test_LTI.msgH:
		  
			print '\n'.join(test_LTI.msgH[:-1])
			#raise AssertionError from msgH[-1]
			raise test_LTI.msgH[-1][0], test_LTI.msgH[-1][1], test_LTI.msgH[-1][2]
