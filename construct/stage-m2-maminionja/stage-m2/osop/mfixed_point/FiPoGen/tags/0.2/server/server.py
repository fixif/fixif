...
def Constant_service(constInter):
	...
	# or get the interval		
		...
		dico = {'error':'',
			'FPF' : str(I.FPF),
			'FPF_image' : BASE_URL+'FPF/' + str(I.FPF) + '.jpg?notation=mlsb&numeric=no&colors=YG&binary_point=yes&label=none&intfrac=no&power2=no'
		}
		return dico
...
def Sum_service(outputFormat):
	...
	latexFPF = "\n".join([f.LaTeX(y_origin=-i*1.3, colors=options["colors"], hatches=((resultFPF.msb,resultFPF.lsb) if options["hatches"] else None)) for i,f in enumerate(F)])
	latexFPF += "\n\t%result\n"+resultFPF.LaTeX( y_origin=-len(F)*1.3-0.5, colors=options["colors"] ) 
	minlsb = min(f.lsb for f in F+[resultFPF])
	maxmsb = max(f.msb for f in F+[resultFPF])
	latexFPF += "\n\t\\draw (%f,%f) -- (%f,%f) [color=black,line width=1pt];"% (-maxmsb-1.2, -len(F)*1.3+0.8, -minlsb+0.2, -len(F)*1.3+0.8)
	if options["axis"]:
		latexFPF += "\n\n\t\\draw (0,1.2) -- (0,%f) [color=red];"%( -len(F)*1.3-0.7,)
