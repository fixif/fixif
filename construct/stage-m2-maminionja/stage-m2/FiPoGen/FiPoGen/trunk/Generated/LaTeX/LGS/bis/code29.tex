...
child{[sibling distance=3.000000cm]
	node(DAdd_3901491) [form] {$F$}
	child{
		node(Add_3901491) [adder] {$+$}
	child{[sibling distance=1cm, level distance=0cm]
		node(DMultt169k1 lsb  10_M) [decM] {$\gg 21 $}
		child{[level distance=0.4cm]
			node(Multt169k1 lsb  10) [mult] {$\times$}
			child{
	...
	child{[sibling distance=1cm, level distance=0cm]
		node(DMultu1k lsb  10_M) [decM] {$\gg 13 $}
		child{[level distance=0.4cm]
			node(Multu1k lsb  10) [mult] {$\times$}
			child{
				node(Cst1) [cst] {703}
			edge from parent node[left] {(-9,-19)}
			}
			child{
				node(Var1) [var] {$u1[k]$}
