...
\begin{tikzpicture}[
    ...
    decM/.style={TreeNode,
		...
		shading angle={45},\begin{figure}[h]
	\centering
	\input{fig/TikZ/.tex}
	\caption{.\label{fig:}}
\end{figure}
...
child{[sibling distance=3.000000cm]
	...
	child{[sibling distance=1cm, level distance=0cm]
		node(DMultt174k1 lsb  9_M) [decM] {$\gg 20 $}
		child{[level distance=0.4cm]
			node(Multt174k1 lsb  9) [mult] {$\times$}
			child{
				node(Cst0) [cst] {1073298}
			edge from parent node[left] {(0,-21)}
			}
			child{
				node(Var0) [var] {$t174[k+1]$}
	...
	child{[sibling distance=1cm, level distance=0cm]
		node(DMultu1k lsb  9_M) [decM] {$\gg 16 $}
		child{[level distance=0.4cm]
