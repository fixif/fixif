...
// /* Clears all entries of a n * n matrix A and frees the memory
//    allocated to that matrix 
// */
// void freeMPFRMatrix(mpfr_t *A, uint64_t n) {
//   uint64_t i, j;
...
void copyMatrix(mpfr_t *B, mpfr_t *A, uint64_t n, uint64_t m) {
  ...
  for (i=0;i<n;i++) {
    for (j=0;j<m;j++) {
      mpfr_set_prec_my(B[i * m + j], mpfr_get_prec(A[i * m + j]));
...
void frobeniusNormLowerBound(mpfr_t frob, mpfr_t *ReA, mpfr_t *ImA, uint64_t n, uint64_t m, mpfr_t scratch) {
  ...
  /* Set precision of scratch variables to output precision + some
     guard bits 
  */
  mpfr_set_prec_my(scratch, mpfr_get_prec(frob) + 2 * ceilLog2(n, m) + 15);
...
int checkFrobeniusNorm(mpfr_t *ReA, mpfr_t *ImA, uint64_t n,uint64_t m, mp_exp_t k, mpfr_t scratch1, mpfr_t scratch2) {
  ...
  /* Set precision of scratch variable scratch1 to precision of k +
     some guard bits. 
  */
  mpfr_set_prec_my(scratch1, 8 * sizeof(k) + 5);
...
void computeAbsoluteValue(mpfr_t res, mpfr_t re, mpfr_t im, mp_exp_t k, mpfr_t scratch) {
  ...
  */
  ...
  /* Set the precision of the scratch variable to the double of the re
     variable. 
  */
  mpfr_set_prec_my(scratch, 2 * mpfr_get_prec(re));
  ...
  */
  ...
  /* Here res is a real */
  mpfr_set_prec_my(scratch, precRes);
...
void accumulateAbsoluteValue(mpfr_t *C, mpfr_t *A, mpfr_t *reB, mpfr_t *imB, uint64_t n,uint64_t m, mp_exp_t k, mpfr_t *scratch) {
  ...
  */
  for (i=0;i<n;i++) {
    for (j=0;j<m;j++) {
      mpfr_set_prec_my(scratch[0], mpfr_get_prec(A[i * m + j]));
      mpfr_set(scratch[0], A[i * m + j], GMP_RNDN); /* exact */
      mpfr_set_prec_my(scratch[1], mpfr_get_prec(C[i * m + j]));
...
void complexMatrixMultiplyAndAdd(mpfr_t *reD, mpfr_t *imD, 
  ...
  */
  for (i=0;i<m;i++) 
  {
    ...
    {
      /* Real part of Dij */
      mpfr_set_prec_my(scratch[0], mpfr_get_prec(reC[i * q + j]));
      if (signP < 0) 
      {
      ...
      for (k=0;k<n;k++) 
      {
	       mpfr_set_prec_my(scratch[1 + 2 * k], (mpfr_get_prec(reA[i * n + k]) + 
	       ...
	       if (signP < 0) 
         {
	       ...
	       mpfr_set_prec_my(scratch[1 + 2 * k + 1], (mpfr_get_prec(imA[i * n + k]) + 
      ...
      mpfr_set_prec_my(scratch[0], mpfr_get_prec(imC[i * q + j]));
      if (signP < 0) 
      {
...
void complexMatrixMultiplyAndAdd_square(mpfr_t *reD, mpfr_t *imD, 
  ...
  for (i=0;i<n;i++) 
  {
    ...
    {
      /* Real part of Dij */
      mpfr_set_prec_my(scratch[0], mpfr_get_prec(reC[i * n + j]));
      if (signS < 0) 
      {
      ...
      /* Imaginary part of Dij */
      mpfr_set_prec_my(scratch[0], mpfr_get_prec(imC[i * n + j]));
      if (signS < 0) 
      {
			 ...
			 mp_exp_t p) 
{
  ...
  */
  scratchV = allocateMPFRVector(2 * n + 1, 64);
  mpfr_init2_my(scratchT1, 64);
  mpfr_init2_my(scratchT2, 64);
...
int checkTwoNorm(mpfr_t *reT, mpfr_t *imT, uint64_t n)
{
  ...
  mpfr_t scratch;
  mpfr_init2_my(scratch, p);    //setting precision of multiplication to 2*prec
  ...
  mpfr_t absTii;
  mpfr_init2_my(absTii, maxT);

  mpfr_t absTgamma;
  mpfr_init2_my(absTgamma, maxT);

  mpfr_t scratch3;
  mpfr_init2_my(scratch3, maxT);
  ...
  for(i = 0; i < n; ++i)
  {
    ...
    {
      printf("Gershgorin's circle is larger than 1 \n ");
      return 0;
