...
class Constant (object):
	...
	Attributes:
		value: real value of the constant (in fact, floating-point value for the moment... Use MPFI in the future)
		FPF: Fixed-Point Format used to represent the constant
		integer: Integer value used to approximate the constant in the given FPF
	"""
	...
	def __init__(self, value, wl= None, signed=True, fpf=None):
		...
		if ( (fpf == None) and (wl == None) ) or ( (fpf != None) and (wl != None) and (fpf.wl != wl) ):
	...
	def approx(self):
		...
		return self._integer * 2**(self._FPF.lsb)
