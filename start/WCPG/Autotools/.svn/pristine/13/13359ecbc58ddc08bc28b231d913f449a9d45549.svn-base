...
//If entry is less than 2^k then we neglet its precision
...
void clapack_matrix_inp_str_d(doublereal *A,int m, int n, FILE *stream)
{
	...
	for(i = 0; i < m; ++i)
	{
	      for(j = 0; j < n; ++j)
	      {
		      fscanf(stream, " %lf ", &(A[i*n + j]));			//&(A[i*kA + j])
	      }
	}
