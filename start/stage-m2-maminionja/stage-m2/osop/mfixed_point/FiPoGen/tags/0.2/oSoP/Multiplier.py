...
class Multiplier(object):
	...
	def __init__(self,cst_val,wl_cst, var_inter, var_name, mult_wl, index, signed=True,RndOff = None, lsb_final=0):
		...
		self._wl = (self._cst_bs.FPF.msb + self._var.FPF.msb +2 -lsb_final) if Multiplier.formatting else mult_wl
		self._rshift_cst = 0
		self._rshift = 0
	...
	def Calc_var_result(self):
		...
		self._local_error = Error(lsb=self._var_result.FPF.lsb-self._rshift, rshift=self._rshift)
	...
	def Tikz(self,osop,tab):
		...
		if (rshift_mult > 0) and (self.result.FPF.wml() == self._result._var_op[self._result._operands.index(self)].FPF.wml()) :
			code += ", level distance=0cm"
		code +="]\n"
		
		# shift computed to align FPF for addition
		#if rshift_add != 0:
		if self.result.FPF.wml() != self._result._var_op[self._result._operands.index(self)].FPF.wml():
			tab += 1
			...
			code += "\t"*tab+"node(D"+repr(self).translate(None,string.punctuation)+") [dec1] {$F$}\n"
			if rshift_mult > 0:
	...
	def Code_C_int(self,R):
		...
		st_fix = "\n\t//Computation of c%d*x%d in register r%d_%d\n"%(self._index, self._index, self.wl,i)
		st_float = ""
		st_fix += "\tc = %d;\n"%(self._cst_bs.integer) #Déclaration de la constante 
		st_fix += "\tr%d = c*x%d"%(i,self._index)
		if self._rshift > 0:
