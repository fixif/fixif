...
class oSoP(object):
	...
	#----------------------------------------------------------------------
	def __init__(self, top, var_final):
		"""Constructeur"""
		self._Top = top
	...
	def Code(self, TYPE, indice=None,FILE = None):
		#self.ReOrder()
		if TYPE == "fix":
			...
			if indice != None:
				file_out = open("Generated/LaTeX/code%d.tex"%(indice), 'w')
			else:
			...
			if self._rshift_final > 0 :
				...
				for j in range(len(R)):
					if R[j] != None:
						i_res = j
						break
				code_int_sop += "\tr%d_%d= r%d_%d >> %d ;\n\treturn r%d_%d;"%(Adder.wl, i,Adder.wl, i_res, self._rshift_final, Adder.wl, i)
				#code_int_sop += "\tdouble res = ((long)r%d)*pow(2,%d);\n\treturn res;"%(i,-self._var_final.lsb)
				
			
			#Génération du code C flottant
			code_float = self.Code("real")
			code_float += "\n\treturn r*powf(2.f,%d);"%(self._var_final.lsb)
			st_arg = ""
			for i in range(n):
				st_arg += "int16_t x%d"%(i)
				if i<n-1:
					st_arg +=","
			d = dict(TP_Int_sop = code_int_sop,TP_Int_dec = code_int_dec,TP_acf_sop = code_acf_sop,TP_acf_dec = code_acf_dec, TP_Float_sop = code_float,TP_Size =st_arg)
			file_out.write(tmplt.safe_substitute(d))
			file_in.close()
			file_out.close()
		else:
