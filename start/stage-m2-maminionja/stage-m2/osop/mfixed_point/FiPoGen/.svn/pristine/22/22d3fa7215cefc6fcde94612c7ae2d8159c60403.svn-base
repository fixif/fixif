...
class FPF(object):
	...
	def minmax(self):
		...
		"""
		if self._signed:
			return ( -2**(self._msb-1), 2**(self._msb-1)-2**self.lsb)
		else:
			return ( 0, 2**self._msb - 2**self.lsb) 
			
	
	def LaTeX(self, y_origin=0, colors=None,  binary_point=False, label='no', notation='mlsb', numeric=False, intfrac=False, power2=False, hatches=None, bits=None, **other_params):
		...
		for m in range( -self._msb-1, min(0,-self._lsb) ):
			st += "\t\\draw (%f,%f) rectangle ++(1,1) [%s,%s] node[midway] {%s};\n"%(m,y_origin, colors[0 if firstSigned else 1], pattern if -m>hatches[0]+1 else '', bits.next() or ('s' if firstSigned else ''))
			firstSigned = False
		for l in range( -min(0,self._msb+1), -self._lsb ):
			st += "\t\\draw (%f,%f) rectangle ++(1,1) [%s,%s] node[midway] {%s};\n"%(l,y_origin, colors[0 if firstSigned else 2], pattern if -l<hatches[1]+1 	else '', bits.next() or ('s' if firstSigned else ''))
			firstSigned=False
