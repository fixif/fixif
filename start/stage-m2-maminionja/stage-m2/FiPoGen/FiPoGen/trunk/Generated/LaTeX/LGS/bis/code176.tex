...
child{[sibling distance=3.000000cm]
	...
	child{[sibling distance=1cm]
		node(DMultu1k lsb  31) [form] {$F$}
		child{[level distance=0.4cm]
			node(Multu1k lsb  31) [mult] {$\times$}
			child{
				node(Cst1) [cst] {22493}
			edge from parent node[left] {(-9,-24)}
			}
			child{
				node(Var1) [var] {$u1[k]$}
			edge from parent node[right] {(8,-7)}
			}
			edge from parent node[left] {(0,-31)}
		}
		edge from parent node[left,sloped,above] {(11,-20)}
	}
	child{[sibling distance=1cm]
		node(DMultt169k1 lsb  18) [form] {$F$}
		child{[level distance=0.4cm]
			node(Multt169k1 lsb  18) [mult] {$\times$}
			child{
				node(Cst0) [cst] {16384}
			edge from parent node[left] {(1,-14)}
			}
			child{
				node(Var0) [var] {$t169[k+1]$}
			edge from parent node[right] {(11,-4)}
			}
			edge from parent node[right] {(13,-18)}
		}
